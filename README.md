...
- Принимает полный российский номер с регионом в свободном формате, например `А123АА77` или `а 123 аа 190`.
- После заказа предлагает сохранить номер; для каждого пользователя хранится до 15 номеров. Команда `/saved` и кнопка «Сохраненные» выводят номера для быстрого заказа.
